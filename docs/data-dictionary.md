...
erDiagram
    ...
    EVENTS {
        ...
        varchar200 title
        text description
        char36 organizer_id "Group 5 user id, no FK"
    ...
    REGISTRATIONS {
        char36 id PK
        char36 event_id FK
        char36 user_id "Group 5 user id, no FK"
        varchar20 status
        datetime created_at
        datetime updated_at
    }
...
| eligibility_rule | JSON NOT NULL | Who may register, e.g. `{"all": true}` or `{"department": "Computing"}`. Stored as one JSON column instead of a separate EventEligibility table |
...
| user_id | CHAR(36) NOT NULL | Group 5 user id, no foreign key |
| status | VARCHAR(20) NOT NULL | see below |
| created_at / updated_at | DATETIME(6) NOT NULL | set automatically |
...
- Only the owning organizer or admin staff can edit, publish or cancel an event.
